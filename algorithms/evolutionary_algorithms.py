...
class DE(GeneralMethod):
    ...
    def step(self, theta, E_old, x_obs, mod, params):
        ...
        theta_new = np.clip(theta_new, a_min=self.clip_min, a_max=self.clip_max)

        # (2. Evaluate)
        E_new = calculate_fitness(x_obs, theta_new, mod, params, self.dist, self.conf)
...
class RevDE(GeneralMethod):
    def __init__(self, conf):
        ...
        R = np.asarray([[1,                            self.gamma,                                 -self.gamma],
                        [-self.gamma,                  1. - self.gamma**2,                         self.gamma + self.gamma**2],
                        [self.gamma + self.gamma**2,   -self.gamma + self.gamma**2 + self.gamma**3,   1. - 2. * self.gamma**2 - self.gamma**3]])

        self.R = np.expand_dims(R, 0) # 1 x 3 x 3

    def proposalREV(self, theta, E=None):
    ...
    def step(self, theta, E_old, x_obs, mod, params):
        # (1. Generate)
        theta_new = self.proposalREV(theta, E_old)
        theta_new = np.clip(theta_new, a_min=self.clip_min, a_max=self.clip_max)
